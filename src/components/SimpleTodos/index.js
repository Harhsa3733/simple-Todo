...
class SimpleTodos extends Component {
  ...
  handleAddTodo = () => {
    ...
    // Check for trailing number *only if there is at least one non-digit before it*
    // e.g. "Buy milk 3" -> ["Buy milk ", "3"]
    const trailingMatch = trimmedTitle.match(/^(.*\D)\s*(\d+)\s*$/)
    ...
    if (trailingMatch) {
      ...
      count = Number(trailingMatch[2]) || count || 1
    }
